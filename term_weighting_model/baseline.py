...
def tfidf_baseline():
    column = "word_seg"
    train = pd.read_csv(from_project_root('data/train_10.csv'))
    test = pd.read_csv(from_project_root('data/test_cut.csv'))
    vec = TfidfVectorizer(ngram_range=(1, 4), min_df=2, max_df=0.8, max_features=2000000, sublinear_tf=1)
    X = vec.fit_transform(train[column])
    X_test = vec.transform(test[column])

    for i in range(10):
        y = (train[str(i)]).astype(int)
        clf = LinearSVC()
        clf.fit(X, y)
